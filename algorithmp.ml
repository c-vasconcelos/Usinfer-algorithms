		...
		| state::w -> 
			if (not (List.exists (fun s -> s = state) states) && not state.is_choice_state) 
				then let transitions = List.flatten (List.map
					(fun a -> 
						print_string ("Next transitions: \n");
						build_transitions class_inv class_init actions a state)
				state.actions) in
...
let generate_typestate class_inv class_init actions =
	print_string "Initial states: \n";
	let initial_state = (build_state class_inv class_init actions) in
		List.iter 
		(fun a -> print_string a.action_name; print_string " ")
		initial_state.actions;
		print_string " \n";
		let delta = (analyse_state class_inv class_init actions [initial_state] [] []) in		
			List.iter 
			(fun t -> 
				print_string ("{(["); 
				(List.iter (fun a -> print_string (" "^a.action_name^" ")) t.state_a.actions); 
				print_string ("], "^t.transition_label^") = [");
				(List.iter (fun a -> print_string (" "^a.action_name^" ")) t.state_b.actions);
				print_string "]}\n") 
			delta; delta
...
let rec annotate_parameters classes action input_parameters output_parameters =
	match input_parameters with
		[] -> print_string "\n"; output_parameters
		| p::input_parameters -> 
			print_string ("		"^p.var_name^" has usage state "^(string_of_int (get_parameter_usage_state_1 classes p action))^"\n"); 
...
let rec annotate_actions classes input_actions output_actions =
	match input_actions with
		[] -> print_string "\n"; output_actions
		| a::input_actions -> 
			print_string ("	"^a.action_name^" has usage state "^(string_of_int (get_action_usage_state_1 classes a))^"\n"); 
...
let rec detect_attributions_2 classes fields actions parameters s output_fields=
	match s with
		{stmt_type = ASSIGN; left_side = [s1]; right_side = [{stmt_type = FIELD; left_side = []; right_side = []; value = v}]} ->
			print_string ("			"^s1.value^" has usage state "^(string_of_int (get_field_usage_state_from_param parameters v))^"\n"); 
		...
		| {stmt_type = IFELSE; left_side = [s1]; right_side = [s2]} -> detect_attributions_2 classes fields actions parameters s1 output_fields; detect_attributions_2 classes fields actions parameters s2 output_fields
		| {stmt_type = SEQ; left_side = [s1]; right_side = [s2]} -> detect_attributions_2 classes fields actions parameters s1 output_fields; detect_attributions_2 classes fields actions parameters s2 output_fields 
...
let main = 
	let annotated_classes = annotate_classes classes [] in
		List.iter
		(fun c -> print_string ("usage lin{"^c.class_name^"; "^(string_of_int (List.hd c.class_usage).usage_state_id)^"} where\n"); List.iter 
			(fun u -> 
				print_string ((string_of_int u.usage_state_id)^" = "^(if u.usage_state_shared then "un" else "lin")^"{"); 
				List.iter (fun b -> print_string (b.action.action_name^"; "); (List.iter (fun ns -> (print_string ((string_of_int ns)^" "))) b.next_states); print_string " + ") u.branches;
				print_string "}\n") 
			c.class_usage)
		annotated_classes;
